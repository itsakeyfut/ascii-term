...
impl VideoProcessor {
    ...
    fn apply_filter(&self, frame: VideoFrame, filter: &VideoFilter) -> Result<VideoFrame> {
        match filter {
            VideoFilter::Resize { width, height } => frame.resize(*width, *height),
            ...
            VideoFilter::EdgeDetection {
                threshold1,
                threshold2,
            } => self.apply_edge_detection(frame, *threshold1, *threshold2),
            VideoFilter::HistogramEqualization => self.apply_histogram_equalization(frame),
            VideoFilter::ColorAdjust {
                brightness,
                contrast,
                saturation,
            } => self.apply_color_adjustment(frame, *brightness, *contrast, *saturation),
            VideoFilter::Rotate { angle } => self.apply_rotation(frame, *angle),
            VideoFilter::Flip {
                horizontal,
                vertical,
    ...
    fn apply_rotation(&self, frame: VideoFrame, angle: f64) -> Result<VideoFrame> {
        let mat = frame.to_opencv_mat()?;
        let center =
            opencv::core::Point2f::new(frame.width as f32 / 2.0, frame.height as f32 / 2.0);
...
mod tests {
    ...
    fn test_filter_chain() {
        ...
        config.filter_chain.push(VideoFilter::Resize {
            width: 640,
            height: 480,
        });
        config.filter_chain.push(VideoFilter::GaussianBlur {
            kernel_size: 5,
            sigma: 1.0,
        });
